...
import Image1 from '../assets/pexels-photo-8348457.jpeg'
...
function Solution() {
  ...
  return (
    <div>
      <SectionHeading title='解决方案' subTitle='解决方案解决方案解决方案解决方案解决方案解决方案' />
        ...
        <div>
          ...
          <ul className='grid gap-3'>
            {[1, 2, 3].map((v, i) => (
              <li
                onClick={() => onClickLi(v)}
                className={`cursor-pointer p-5 justify-between rounded border border-zinc-100 ${
                  v === activeLi && 'bg-zinc-100'
                }`}
                key={v}>
                <p className='text-lg font-bold'>构建简单的生态系统{i + 1}</p>
